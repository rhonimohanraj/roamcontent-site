/* Roam Content Co. — location personalization
 *
 * Detects a visitor's approximate position from their IP address, finds the
 * nearest service city (Brandon, Winnipeg, or Regina), and personalizes the
 * homepage in place. The dedicated /brandon/, /winnipeg/, /regina/ pages
...
(() => {
  ...
  const LOCATIONS = {
    brandon: {
      ...
      lng: -99.9501,
      url: '/brandon/',
      eyebrow: 'Now roaming · Brandon · Westman · Manitoba',
    ...
    winnipeg: {
      ...
      lng: -97.1384,
      url: '/winnipeg/',
      eyebrow: 'Now roaming · Winnipeg · Manitoba',
    ...
    regina: {
      ...
      lng: -104.6189,
      url: '/regina/',
      eyebrow: 'Now roaming · Regina · Saskatchewan',
      serving: 'Regina & southern Saskatchewan',
    },
  };

  const DEFAULT_KEY = 'brandon';
  const CACHE_KEY = 'roam-geo';
  ...
  const personalize = (key) => {
    ...
    document.querySelectorAll('[data-geo-cta]').forEach((el) => {
      ...
      if (label) {
        label.textContent = `Planning something near ${loc.name}? See our ${loc.name} page`;
      }
      el.hidden = false;
    });
